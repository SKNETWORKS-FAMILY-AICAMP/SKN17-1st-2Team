...
def setup_database():
    try:
        ...
        conn.commit()
        print(f"MySQL 데이터베이스 '{db_config['database']}' 연결 및 'kia_faq' 테이블 설정 완료.")
        return conn, cursor
    except Error as err:
...
def insert_data_to_db(cursor, conn, data):
    ...
    try:
        cursor.execute(insert_query, (data['keyword'], data['title'], data['content']))
        conn.commit()
...
def crawl_kia_faq_for_keyword(keyword, driver, cursor, conn):
    faq_data = []
        ...
        try:
            ...
            for i in range(total_questions):
                ...
                try:
                    ...
                    item = {'keyword': keyword, 'title': title, 'content': content}
                    faq_data.append(item)
                    print(f"데이터 수집 완료 ({len(faq_data)}/{total_questions}): {title[:30]}...")
                    
                    if conn and cursor:
                        insert_data_to_db(cursor, conn, item)
...
def main_crawler():
    ...
    try:
        ...
        for keyword in search_keywords:
            data = crawl_kia_faq_for_keyword(keyword, driver, cursor, conn)
            if not data:
